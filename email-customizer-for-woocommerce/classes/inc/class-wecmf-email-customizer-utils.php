...
class WECMF_Utils {
	...
	public static function get_thwecmf_styles(){
        ...
        // Set min-width for mobile screens.
	...
	/**
	 * Restore WooCommerce default email header/footer hooks after custom ones are used
	 *
	 * @param object $mailer WC_Email instance
	 */
	
	public static function restore_woocommerce_hooks($email)
		{
			// Only restore if our custom hooks are currently active
			if (!has_action('woocommerce_email_header', ['WECMF_Utils', 'thwecmf_email_header'])) {
				return; // Custom hooks not active, nothing to restore
			}

			// Remove our custom hooks
			remove_action('woocommerce_email_header', ['WECMF_Utils', 'thwecmf_email_header'], 10);
			remove_action('woocommerce_email_footer', ['WECMF_Utils', 'thwecmf_email_footer'], 10);

			// Re-add WooCommerce defaults from the email object
			$mailer = WC()->mailer();
			if (is_object($mailer) && method_exists($mailer, 'email_header')) {
	...
	/**
	 * Replace WooCommerce default email header/footer hooks with custom ones
	 *
	* @param object $email WC_Email instance
	*/
	public static function manage_woocommerce_hooks($email)
		{
			
			// This ensures clean slate regardless of which object registered them
			// remove_action( 'woocommerce_email_header', [$email, 'email_header'] ,10);
			// remove_action( 'woocommerce_email_footer', [$email, 'email_footer'] ,10);
			remove_all_actions( 'woocommerce_email_header' );
			remove_all_actions( 'woocommerce_email_footer' );

			// Add ONLY the plugin's custom header & footer handlers
			add_action('woocommerce_email_header', ['WECMF_Utils', 'thwecmf_email_header'], 10, 2);
			add_action('woocommerce_email_footer', ['WECMF_Utils', 'thwecmf_email_footer'], 10, 2);
		}
	...
	* @param object $email WC_Email instance
	*/
	public static function thwecmf_email_header($email_heading, $email){
	 ...
	 * @param object $email WC_Email instance
	 */
	public static function thwecmf_email_footer($email){
			?>
			<table width="100%">
				<tr>
					<td align="center" valign="top">
